...
<main class="site-main" id="main-content" style="background:#F9FBFC;min-height:70vh;">
    <section style="padding:3rem 1rem 5rem;">
        <div style="max-width:560px;margin:0 auto;">

            <?php if (!is_user_logged_in()): ?>

                <div style="background:#fff;border:1px solid #EAF0F6;border-radius:16px;padding:2.5rem;text-align:center;box-shadow:0 8px 20px rgba(11,35,71,0.05);">
            ...
            <?php elseif (!current_user_can('edit_posts')): ?>
                ...
                <div style="background:#fff;border:1px solid #EAF0F6;border-radius:16px;padding:2.5rem;text-align:center;">
            ...
            <?php elseif (isset($_GET['submitted'])):
                ...
                <div style="background:#fff;border:1px solid #EAF0F6;border-radius:16px;padding:2.5rem;text-align:center;box-shadow:0 8px 20px rgba(11,35,71,0.05);">
                    ...
                    <div style="background:#F0F4F8;border-radius:12px;padding:1.5rem;text-align:left;margin-top:1rem;">
                        <strong style="color:#0B2347;display:block;margin-bottom:0.75rem;">📱 Step 2: Ask for the review while you're still in the driveway</strong>
                        <p style="font-size:0.92rem;color:#3D6491;line-height:1.6;background:#fff;border:1px dashed #C9D6E4;border-radius:8px;padding:0.9rem;" id="hwh-review-msg"><?php echo esc_html($msg); ?></p>
            ...
            <?php else: ?>
                ...
                <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" enctype="multipart/form-data"
                      style="background:#fff;border:1px solid #EAF0F6;border-radius:16px;padding:2rem;display:flex;flex-direction:column;gap:1.25rem;box-shadow:0 8px 20px rgba(11,35,71,0.05);">
                    <input type="hidden" name="action" value="hwh_add_job">
                    <?php wp_nonce_field('hwh_add_job', 'hwh_add_job_nonce'); ?>

                    <div>
                        <label style="font-weight:700;color:#0B2347;display:block;margin-bottom:6px;">📷 Job photos</label>
                        <input type="file" name="job_photos[]" accept="image/*" multiple
                               style="width:100%;padding:0.9rem;border:2px dashed #C9D6E4;border-radius:10px;background:#F9FBFC;font-size:1rem;">
                        <p style="font-size:0.85rem;color:#6B87A6;margin:6px 0 0;">3–4 clear shots: before, during, after, and the finished install.</p>
                    </div>

                    <div>
                        <label style="font-weight:700;color:#0B2347;display:block;margin-bottom:6px;">📍 Neighborhood</label>
                        <select name="job_location" required style="width:100%;padding:0.9rem;border:1px solid #C9D6E4;border-radius:10px;font-size:1.05rem;background:#fff;">
                    ...
                    <div>
                        <label style="font-weight:700;color:#0B2347;display:block;margin-bottom:6px;">🔧 What did we do?</label>
                        <select name="job_service" required style="width:100%;padding:0.9rem;border:1px solid #C9D6E4;border-radius:10px;font-size:1.05rem;background:#fff;">
                    ...
                    <div>
                        <label style="font-weight:700;color:#0B2347;display:block;margin-bottom:6px;">📝 Quick note (2–3 sentences)</label>
                        <textarea name="job_note" rows="4" required
                                  ...
                                  style="width:100%;padding:0.9rem;border:1px solid #C9D6E4;border-radius:10px;font-size:1rem;line-height:1.5;"></textarea>
                    </div>

                    <div style="border-top:1px solid #EAF0F6;padding-top:1.25rem;">
                        <strong style="color:#0B2347;display:block;margin-bottom:0.75rem;">For the review request (optional)</strong>
                        <div style="display:flex;gap:1rem;flex-wrap:wrap;">
                            <input type="text" name="customer_name" placeholder="Customer first name" style="flex:1;min-width:140px;padding:0.9rem;border:1px solid #C9D6E4;border-radius:10px;font-size:1rem;">
                            <input type="tel" name="customer_phone" placeholder="Customer cell #" style="flex:1;min-width:140px;padding:0.9rem;border:1px solid #C9D6E4;border-radius:10px;font-size:1rem;">
                        </div>
                        <p style="font-size:0.85rem;color:#6B87A6;margin:6px 0 0;">Only used to open a pre-written review-request text on your phone — never shown on the website.</p>
                    </div>
