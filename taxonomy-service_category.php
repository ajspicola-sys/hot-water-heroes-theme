<?php
/**
 * Hot Water Heroes — Service Category Archive Template
 * Renders services belonging to a specific service category (e.g. Water Heater Services).
 * Solves the template fallback bug and eliminates 404 pagination crawl errors.
 */
get_header();
...
<main class="site-main" id="main-content">
    ...
    <!-- ── Page Hero ─────────────────────────────────────────────── -->
    <section class="hwh-hero hwh-hero--inner" aria-label="<?php echo esc_attr( $term->name ); ?>">
        <div class="hwh-hero__overlay" aria-hidden="true"></div>
        <div class="hwh-hero__grid" aria-hidden="true"></div>
        <div class="hwh-section-inner" style="position:relative;z-index:2;text-align:center;padding-top:3rem;padding-bottom:3rem;">
            ...
            <nav class="breadcrumbs breadcrumbs--hero" aria-label="Breadcrumb" style="display:inline-block;margin-bottom:1.5rem;" itemscope itemtype="https://schema.org/BreadcrumbList">
                <ol class="breadcrumbs__list" style="display:flex;gap:0.5rem;justify-content:center;list-style:none;padding:0;margin:0;font-size:0.9rem;opacity:0.85;">
                    ...
                    <li class="breadcrumbs__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                        <a href="<?php echo esc_url(home_url('/services/')); ?>" itemprop="item" style="color:#fff;text-decoration:none;"><span itemprop="name">Services</span></a>
                    ...
                    <li class="breadcrumbs__item breadcrumbs__item--current" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" aria-current="page">
                        <span itemprop="name" style="color:#fff;font-weight:600;"><?php echo esc_html( $term->name ); ?></span>
                        <meta itemprop="position" content="3">
                    </li>
                </ol>
            </nav>

            <span class="hwh-label hwh-label--white" style="display:block;margin:0 auto 1rem auto;width:max-content;">Service Category</span>
            <h1 class="hwh-section-title hwh-section-title--white" style="margin-bottom:1rem;">
                <?php echo esc_html( $term->name ); ?><br><em>Tampa Bay Trusts</em>
            </h1>
            <p class="hwh-section-desc hwh-section-desc--muted" style="max-width:650px;margin:0 auto;">
                <?php 
                if ( ! empty( $term->description ) ) {
                    echo esc_html( $term->description );
                } else {
                    echo 'Licensed plumbers ready same-day — professional services backed by our 100% satisfaction guarantee.';
                }
                ?>
            </p>
    ...
    <!-- ── Services Grid — fetches ONLY services in this category ───── -->
    <section class="hwh-services" aria-label="<?php echo esc_attr( $term->name ); ?> grid" style="padding-top:5rem;padding-bottom:5rem;">
        <div class="hwh-section-inner">

            <?php
            // Fetch only services in the current taxonomy term
            $services_query = new WP_Query([
                'post_type'      => 'service',
                'post_status'    => 'publish',
                'posts_per_page' => -1, // Fetch all belonging to this term (no pagination = zero 404s)
                'orderby'        => 'menu_order',
                'order'          => 'ASC',
                'no_found_rows'  => true,
                'tax_query'      => [
            ...
            <?php else: ?>
                <p style="text-align:center;padding:4rem 0;font-size:1.1rem;color:#3D6491;">
                    No services found in this category. <a href="<?php echo esc_url(home_url('/contact/')); ?>">Contact us directly.</a>
                </p>
            <?php endif; ?>
